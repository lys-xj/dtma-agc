...
def load_graph(dataset, k=None, graph_path=None):
    if graph_path:
        path = graph_path
    elif k:
    ...
    print(f"Graph file path: {os.path.abspath(path)}")
    ...
    if edges_unordered.shape[1] == 3:
        edges = np.array(list(map(idx_map.get, edges_unordered[:, :2].flatten())),
                         dtype=np.int32).reshape(edges_unordered[:, :2].shape)
